...
/* WEBPACK VAR INJECTION */(function(Buffer) {__webpack_require__(/*! fast-text-encoding */ "../node_modules/fast-text-encoding/text.min.js");
...
var browser = new BufferCoder();

class Packet {
  constructor() {
    this.HEADER_LEN_SIZE = 4, this.HEADER_LEN_TYPECODE = 2, this.HEADER_LEN_ENCRYPT = 1, this.HEADER_LEN_PLACEHOLDER = 1, this.HEADER_LEN_TOTAL = this.HEADER_LEN_SIZE * 2 + this.HEADER_LEN_TYPECODE + this.HEADER_LEN_ENCRYPT + this.HEADER_LEN_PLACEHOLDER, this.buffer = Buffer.alloc(0);
  }

  nodeEncode(a) {
    var b = Buffer.alloc(this.HEADER_LEN_TOTAL);
    var c = Buffer.from(a, 'utf8');
    var d = c.length + this.HEADER_LEN_TOTAL + 1;
    return b.writeInt32LE(d - this.HEADER_LEN_SIZE, 0), b.writeInt32LE(d - this.HEADER_LEN_SIZE, 4), b.writeInt16LE(689, 8), Buffer.concat([b, c], d);
  }

  browserEncode(a) {
    return browser.Encode(a);
  }

  Encode(a) {
    return util.isBrowser() ? this.browserEncode(a) : this.nodeEncode(a);
  }

  nodeDecode(a, b) {
    this.buffer = Buffer.concat([this.buffer, a]);

    for (var c = 0; this.buffer.length > 0; c = 0) {
      if (0 === c) {
        if (this.buffer.length < this.HEADER_LEN_SIZE) return;
        c = this.buffer.readInt32LE(this.HEADER_LEN_SIZE), this.buffer = this.buffer.slice(this.HEADER_LEN_SIZE);
      }

      if (this.buffer.length < c) return;
      var d = this.buffer.slice(this.HEADER_LEN_SIZE * 2, c - 1).toString('utf8');
      this.buffer = this.buffer.slice(c), b(d);
    }
  }
  ...
  Decode(a, b) {
    util.isBrowser() ? this.browserDecode(a, b) : this.nodeDecode(a, b);
  }

}

module.exports = new Packet();
/* WEBPACK VAR INJECTION */}.call(this, __webpack_require__(/*! ./../build/node_modules/buffer/index.js */ "./node_modules/buffer/index.js").Buffer))
